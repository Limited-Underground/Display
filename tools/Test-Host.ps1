...
$output = Join-Path $buildDirectory 'critical_alert_export_tests.exe'
$sources = @(
...
if ($LASTEXITCODE -ne 0) {
    throw "Critical alert exporter compilation failed with exit code $LASTEXITCODE."
}

& $output
if ($LASTEXITCODE -ne 0) {
    throw "Critical alert exporter tests failed with exit code $LASTEXITCODE."
}
