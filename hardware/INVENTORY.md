...
| --- | --- | --- | --- | --- |
| Round gauge display | 2 x Waveshare ESP32-S3-Touch-AMOLED-1.75-B candidates, procurement mapping SKU 31262, in protective case | Owner reports two on hand; one connected candidate received bounded read-only testing while the second remained disconnected and untouched | Candidate only; package and underside labels plus one powered demo are recorded. Esptool 5.3.1 confirmed ESP32-S3, 16 MB flash, disabled Secure Boot and flash encryption; Secure Download Mode was not explicitly captured. The sole privacy-safe USB interface returned after each reset, and an exact-size private full-flash capture was hashed outside Git. Exact board revision, box/unit association, visual post-reset application return, manual recovery, peripherals, flashing, and OpenGauge behavior remain untested | Follow `hardware/WAVESHARE_31262_BRINGUP.md`: keep the second unit untouched, associate a public-safe ID, visually confirm the previously photographed powered demo, prove cold BOOT-strap recovery, then test one verified vendor image and measure display/touch/power/IMU and synthetic-gauge behavior |
