...
- A canonical 640-byte `OOC0` outbox checkpoint now represents up to eight queued/in-flight exact frames with attempts and remaining lifetime/action time, bound to a nonzero configuration fingerprint. Atomic boot-only live outbox export/import reconstructs queued retry readiness, in-flight ACK timeout, and maximum lifetime against a new monotonic origin; prepared sends, corrupt/incompatible records, nonempty runtime, and unrepresentable timers fail closed. Seven codec plus five integration groups, the full 29-executable matrix, and 100 focused repeats pass. Coordinated ACK/authorization generation, durable two-slot target storage, power-loss, authenticated integrity, and rollback resistance remain.
