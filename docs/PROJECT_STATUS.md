...
- A canonical 640-byte `OOC0` outbox checkpoint codec now represents up to eight queued/in-flight exact frames with attempts and remaining lifetime/action time, bound to a nonzero configuration fingerprint. Seven host groups plus 100 repeats enforce canonical padding, event uniqueness, state/timing/frame validity, CRC, and atomic decode. Live outbox export/import and coordinated durable storage remain.
