...
- The logical peer registry holds eight role-scoped entries and one exact local approval window; stores only opaque logical IDs, secure-key handles, and epochs; authorizes exact peer+handle+channel+permission; and supports revoke/forget/replacement/rotation with atomic snapshots. Its fixed 256-byte `OPA0` record preserves active/revoked peers and authorization epochs, refuses pending approvals, and imports atomically only into a clean boot registry. The registry and checkpoint each have eight host groups and 100 repeat runs; the full 33-executable matrix passes. Discovery UI/local-presence proof, protected raw-key storage/crypto, coordinated `OPA0`/`OCR0` recovery, rate limits, ESP-NOW binding, rollback protection, and physical provisioning remain.
