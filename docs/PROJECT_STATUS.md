...
- The target recovery-adapter plan now defines exact two-slot semantics, protected opaque-handle resolution, a separate trusted-generation source, boot and save ordering, reset/replacement coordination, and a physical interruption/endurance acceptance matrix. It explicitly records that the connected MeshCore radios cannot validate an OpenGauge ESP-IDF storage path and that no target implementation exists yet.
