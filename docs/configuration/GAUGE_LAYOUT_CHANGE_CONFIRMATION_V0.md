...
- `stage(request_id, layout, now_ms)` accepts one nonzero request ID, validates
  canonical layout content without trusting its generation, and records the
  monotonic opening time.
...
Only one request may be pending. Request IDs correlate local UI state; they are
not credentials, signatures, authorization, or replay protection for a remote
protocol.
...
5. source authorization and integrity before any remote or imported proposal;
6. restart reconciliation and clear operator status for uncertain persistence;
7. an ESP-IDF storage backend and physical interruption/endurance evidence.
...
Nine deterministic host groups cover lifecycle/policy, validation, one-pending
admission, exact confirmation, unchanged-write suppression, mismatch/cancel,
the exact expiry boundary, clock rollback, ordinary failure, and applied
uncertainty followed by restart reconciliation. The suite passes 100/100
focused repeats and the complete 44-executable host matrix under strict GCC
warnings-as-errors.

This evidence does not validate physical input, display readability, concurrent
target tasks, authenticated configuration, ESP-IDF storage, power cuts, flash
endurance, or any supported hardware.
