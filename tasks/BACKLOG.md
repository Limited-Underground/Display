...
| OG-010C | done | Cache-to-radio gateway publisher | A cooperative composition performs bounded cache full/incremental cursor polls, maps only four registered IDs, counts/skips unknowns, safely resets source/publication baselines on cache epoch change, encodes one 96-byte packet, attempts one nonblocking send, and commits local acceptance/rejection. Six host groups cover initial/unchanged mapping, clear plus same-value reload, same-sequence local retry, paced 3+1 sync, stale without repoll, and accepted injected loss followed by an exact receiver gap |
