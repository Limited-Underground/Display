...
| Proven so far | A 52-executable Windows host-test matrix and limited three-radio alert/acknowledgement evidence |
...
No hardware is supported yet. Candidate equipment includes two owner-reported-on-hand Waveshare ESP32-S3 1.75-inch round touch displays, an owner-reported-ordered ESP32-S3 development board, an owner-reported-ordered Wio Tracker L1 Pro GNSS/LoRa unit, and an on-hand consumer OBD-II adapter. The active battery-free USB candidate `OG-DISP-001` now has bounded ESP32-S3/16 MB/security metadata, a private exact-size backup outside Git, passed cold BOOT recovery, visibly ran a hash-pinned official HelloWorld image, and returned to a visually good demo after a verified write of the hash-pinned official factory image. Labeled `OG-DISP-002` remained disconnected and untouched. No touch or other peripheral, resource/timing/stability, power/heat, paired-independence, OpenGauge-firmware, or compatibility result exists, so every item remains unverified. See the dated [Phase-B](tests/hardware/OG-012A-PHASE-B-2026-08-14.md) and [Phase-C](tests/hardware/OG-012A-PHASE-C-2026-08-14.md) evidence.
