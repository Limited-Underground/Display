...
- Coordinated restart groundwork now adds a host-tested [fixed 960-byte `OCR0` generation envelope](docs/integration/CRITICAL_ALERT_RECOVERY_CHECKPOINT_V0.md) containing the exact ACK replay/authorization checkpoint and exact outbox checkpoint together. This is the canonical unit for the next dual-import/two-slot-store step; it is not yet durable storage.
