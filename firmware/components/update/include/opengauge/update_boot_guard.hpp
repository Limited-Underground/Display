...
enum class TrialHealth : std::uint32_t {
    ...
    core_io_healthy = 1U << 2U,
    configuration_loaded = 1U << 3U,
    display_healthy = 1U << 4U,
    radio_healthy = 1U << 5U,
};
...
inline constexpr std::uint32_t kAllTrialHealthBits =
    ...
    health_bit(TrialHealth::core_io_healthy) |
    health_bit(TrialHealth::configuration_loaded) |
    health_bit(TrialHealth::display_healthy) |
