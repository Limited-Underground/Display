...
bool known_disposition(AlertAckDisposition disposition) {
    return disposition == AlertAckDisposition::accepted ||
           disposition == AlertAckDisposition::rejected;
}

bool known_reason(AlertAckReason reason) {
...
bool known_state(AlertState state) {
    return state == AlertState::asserted || state == AlertState::cleared;
}
...
AlertAckCodecError validate_critical_alert_ack(
    ...
    if (acknowledgement.observed_alert_age_ms > kMaximumObservedAgeMs) {
        return AlertAckCodecError::age_out_of_range;
    }
    return AlertAckCodecError::none;
}
...
AlertAckDecodeResult decode_critical_alert_ack(
    ...
    acknowledgement.disposition =
        static_cast<AlertAckDisposition>(data[6]);
    ...
    return {AlertAckCodecError::none, acknowledgement};
}
