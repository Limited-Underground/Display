...
enum class FakeSystemRecoveryWriteBehavior : std::uint8_t {
    normal = 0,
    fail_before_write,
...
class FakeCriticalAlertSystemRecoveryStorage final
...
private:
    std::array<
        std::array<std::uint8_t, kCriticalAlertSystemRecoveryCheckpointBytes>, 2>
        slots_{};
    std::array<bool, 2> present_{};
    std::array<bool, 2> fail_read_{};
    std::array<FakeSystemRecoveryWriteBehavior, 2> next_write_{};
